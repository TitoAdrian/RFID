...
/** 
 ...
 <p>
 Captures the EP CR PC QT RI responses to form a TransponderData instance.
 ...
 - A new response header is received that has been seen before (moreAvailable = true). Just received the start of the next transponder response
...
public class TransponderResponder
{
    ...
    */
    ...
    /**
     Gets the last received transponder QT


     Gets the QT part of an inventory response from a transponder
     or null if QT output is not enabled
    ...
    private void setQt(Integer value)
    {
        privateQt = value;
    }
    ...
    */
    ...
    /**
     * The list of transponder data headers already received - used to detect a complete response
     * Now that all transponder responses such as EPC or CRC are optional we will assume that
     * when the same header appears again that this is from the next transponders response
     */
    private HashSet<String> mTransponderHeadersSeen = new HashSet<String>();
    ...
    */
    ...
    {
        boolean result = true;
        if( mTransponderHeadersSeen.contains(header))
        {
            // command completed successfully
            transponderComplete(true);
        }

        mTransponderHeadersSeen.add(header);
        ...
        else if ("QT".equals(header))
        {
            // received current transponder PC value in hex
            setQt(Integer.parseInt(value, 16));
        }
    ...
    */
    ...
    {
        if (!mTransponderHeadersSeen.isEmpty())
        {
