...
//					Log.d(TAG, String.format("CommandCondition notified (%s)\n\"%s\"", this.getLastCommandLine(),line));
	...
	*/
						...
						case BluetoothReaderService.MESSAGE_STATE_CHANGE:
							...
							switch (msg.arg1)
							{
								case BluetoothReaderService.STATE_CONNECTED:
                                    ...
                                    commander.updateDeviceProperties();
    ...
    /**
     * Update the AsciiCommander's DeviceProperties for the currently connected reader
     */
    private void updateDeviceProperties()
    {
        // Ensure that Synchronous Version commands will run
        boolean isSynchronousResponderNeeded = !this.getHasSynchronousResponder();
        if( isSynchronousResponderNeeded )
        {
        ...
        do
        {
            ...
            finally
            {
                ...
                {
                    String serialNumber = this.informationCommand.getSerialNumber();
                    this.deviceProperties = new DeviceProperties(serialNumber);
                    attempts = 0;
                }
                else
                {
        ...
        // Restore responder chain to initial state
        if( isSynchronousResponderNeeded )
        {
